...
public final class CuratorUtils {
    ...
    public static void clearRegistry() {
        ...
        System.out.println(String.format("服务端（Provider）所有注册的服务都被清空:[{}]", registeredPathSet.toString()));
    }
    ...
    private static void registerWatcher(CuratorFramework zkClient, String serviceName) {
        String servicePath = ZK_REGISTER_ROOT_PATH + "/" + serviceName;
        CuratorCache curatorCache = CuratorCache.build(zkClient, servicePath, CuratorCache.Options.COMPRESSED_DATA);
        PathChildrenCacheListener pathChildrenCacheListener = (curatorFramework, pathChildrenCacheEvent) -> {
          List<String> serviceAddresses = curatorFramework.getChildren().forPath(servicePath);
          serviceAddressMap.put(serviceName, serviceAddresses);
      };
        curatorCache.listenable().addListener(CuratorCacheListener.builder().forPathChildrenCache(zkClient, pathChildrenCacheListener).build());
        try {
          curatorCache.start();
        } catch (Exception e) {
            e.printStackTrace();
